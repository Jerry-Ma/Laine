...
function getAlbumArt(url, callback) {
    ...
    _httpSession.queue_message(request, function(_httpSession, message) {
        if (message.status_code !== 200) {
          ...
          var icon = GdkPixbuf.Pixbuf.new_from_inline(albumart, false);
          callback(null, icon);
        };
    });
};
...
const MPRISStream = new Lang.Class({
    ...
    _init: function(parent, dbusPath, pid, dbus, paconn){
        ...
        this._getAppByPid = function(){
            //    set by pid
            let _app = WindowTracker.get_app_from_pid(this._procID);
            if(_app == null){
                //Doesn't have an open window, lets check the tray.
                let trayNotifications = Main.messageTray.getSources();
                for(let i = 0; i < trayNotifications.length; i++)
                    if(trayNotifications[i].pid == this._procID)
                        _app = trayNotifications[i].app;
            }
            return _app;
        };
    ...
    _hdlDesktopEntry: function(conn, result){
        ...
        try{
            ...
            app = Shell.AppSystem.get_default().lookup_app(dName+".desktop");
        } catch (err){
            app = this._getAppByPid();
        }
        let icon;
        if(app != null){
            let info = app.get_app_info();
            this._label.text = info.get_name();
            icon = new St.Icon({style_class: 'mpris-stream-icon'});
            icon.set_gicon(info.get_icon());
        } else {
            icon = new St.Icon({icon_name: 'package_multimedia', style_class: 'mpris-stream-icon'});
            this._dbus.call(this._path, '/org/mpris/MediaPlayer2', "org.freedesktop.DBus.Properties", "Get",
        ...
        if(meta == null || meta.n_children() == 0){
            ...
            if('mpris:artUrl' in metaD){
                ...
                if(GLib.file_test(iconPath, GLib.FileTest.EXISTS)){
                    ...
                    this._albumArt.set_icon_name('equaliser');
                }
                this._albumArt.show();
            } else {
                this._albumArt.hide();
